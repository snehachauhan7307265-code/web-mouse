...
import { AlertTriangle, ShieldCheck, X, Cast, FolderUp, Power, Check } from 'lucide-react';
      ...
      <div className="w-full max-w-sm bg-zinc-950 border border-zinc-800 rounded-3xl p-6 space-y-4 shadow-2xl">
        ...
        <div>
          ...
          <h3 className="text-base font-bold text-white mt-0.5">
            {intent.intent === 'start_projection'
              ? 'Project Screen'
              : intent.intent === 'send_file'
              ? 'Send File'
              : 'Execute Command'}
          </h3>
          <p className="text-xs text-zinc-400 mt-2 leading-relaxed">
            {intent.intent === 'start_projection'
              ? `Project your screen to ${targetDeviceName}?`
              : intent.intent === 'send_file'
              ? `Send file to ${targetDeviceName}?`
              : `Execute ${intent.intent} on ${targetDeviceName}?`}
          </p>
        </div>
        ...
        <div className="p-3 bg-zinc-900/80 rounded-2xl border border-zinc-800 flex items-start gap-2.5 text-xs text-zinc-400">
          ...
          <span>
            Target: <strong className="text-white">{targetDeviceName}</strong>. Safe execution protocol enforced.
          </span>
        </div>
